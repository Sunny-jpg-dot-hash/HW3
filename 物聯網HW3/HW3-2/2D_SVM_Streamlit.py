...
# 生成圓形分布的資料點
def generate_circular_data(num_points, radius):
...
def plot_3d(x1, x2, x3, labels, coef, intercept):
    ...
    fig.update_layout(scene=dict(xaxis_title='x1', yaxis_title='x2', zaxis_title='x3'),
                      title='3D 圖形顯示圓形分布資料及 SVM 分隔超平面')
    return fig
...
if __name__ == "__main__":
    ...
    radius = st.slider("Radius for Circular Distribution", min_value=2.0, max_value=15.0, value=8.0)
